...
_executor = ThreadPoolExecutor(
    max_workers=int(os.getenv("OCR_WORKERS", "4")),  # increased for parallel requests
...
_engine_lock = None   # initialized in lifespan to avoid asyncio issues at module load
_engine_mutex = __import__("threading").Lock()   # thread-safe for executor pool
...
async def run_ocr(
    ...
    engine_override: Optional[str] = Query(None, alias="engine",
        description="OCR engine override (currently only trocr is supported)"),
    db: Session = Depends(get_db),
):

...
class RubricMatcher:
    """
    Two-mode rubric matching:
    1. Zero-shot NLI (default, no training) — uses DeBERTa NLI model
    2. Fine-tuned BERT classifier (if training data available)
    """
    ...
    def evaluate_rubric(
        self,
        student_answer: str,
        rubric_criteria: list,   # list of {"criterion": str, "marks": float}
        threshold: float = 0.5
    ) -> RubricResult:
        """
        ...
        [
            ...
            {"criterion": "difference between AI and ML", "marks": 1.5},
        ]

        threshold: NLI entailment probability above which criterion is "present".
        """
    ...
    def _zero_shot_check(
        ...
        # run zero-shot classification with each criterion as a candidate label
        result = pipe(
            student_answer,
    ...
    def _finetuned_check(self, student_answer: str, criteria: list) -> dict:
        """Placeholder for fine-tuned binary classifier per criterion."""
        # Fall back to zero-shot if no model loaded
    ...
    def train_finetuned(
        ...
        """
        Fine-tune BERT for rubric detection.
        ...
        logger.info(f"Rubric classifier saved to {output_dir}")
